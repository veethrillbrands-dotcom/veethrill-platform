...
export default async function VendorInvoicesPage() {
  ...
  const outstanding = invoices.filter((i) => !["PAID", "DRAFT"].includes(i.status)).reduce((s, i) => s + i.total, 0);

  return (
    <div className="flex flex-col h-full overflow-hidden">
      <PortalTopbar title="My Invoices" />
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 font-bold text-gray-900 text-[14px]">Invoice History</div>
          {invoices.length === 0 ? (
            <div className="text-center text-gray-400 py-10 text-[13px]">No invoices issued yet.</div>
          ) : (
