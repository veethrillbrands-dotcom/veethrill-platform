...
export async function POST(req: Request) {
  try {
    ...
    const { title, type, location, totalUnits, priceFrom, priceTo, developer, completionDate, yieldEstimate, targetInvestor, highlights } = body;
    ...
    const dossier = await db.crmDossier.create({
      data: {
        title, type: type || "Luxury Residential", location,
        ...
        targetInvestor, highlights, status: "Active",
      },
    });
