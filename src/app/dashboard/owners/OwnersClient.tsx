...
function EditOwnerModal({ owner, onClose }: { owner: Owner; onClose: () => void }) {
  const router = useRouter();
  const [saving, setSaving] = useState(false);
  const [success, setSuccess] = useState(false);
  const [form, setForm] = useState({
    firstName: owner.user.firstName, lastName: owner.user.lastName,
    phone: owner.user.phone ?? "", bankName: owner.bankName ?? "",
    bankAccountNumber: owner.bankAccountNumber ?? "", bankAccountName: owner.bankAccountName ?? "",
  });
  const set = (k: string, v: string) => setForm((f) => ({ ...f, [k]: v }));
  ...
  if (success) return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-3xl p-10 flex flex-col items-center gap-4 shadow-2xl">
        <CheckCircle size={32} className="text-emerald-600" />
        <div className="text-[17px] font-bold text-gray-900">Owner Updated!</div>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md mx-4 overflow-hidden max-h-[90vh] flex flex-col">
        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between flex-shrink-0" style={{ background: "var(--navy)" }}>
          <div className="text-[15px] font-bold text-white">Edit Owner</div>
          <button onClick={onClose} className="text-white/60 hover:text-white"><X size={20} /></button>
        </div>
        <div className="p-6 space-y-4 overflow-y-auto">
          <div className="grid grid-cols-2 gap-3">
  ...
  return (
    <>
      <div className="flex items-center gap-1">
        ...
        <button onClick={() => setEditing(true)} title="Edit owner"
      ...
      {editing && <EditOwnerModal owner={owner} onClose={() => setEditing(false)} />}
    </>
  );
}
