...
type Booking = {
  ...
  specialRequests: string | null; checkInCode: string | null;
...
function NewBookingModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    checkIn: "", checkOut: "", nightlyRate: "", guestCount: "1", source: "DIRECT", specialRequests: "",
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden max-h-[90vh] flex flex-col">
        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between flex-shrink-0" style={{ background: "var(--navy)" }}>
          <div className="text-[15px] font-bold text-white">New Booking</div>
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="number" value={form.guestCount} min="1" onChange={(e) => set("guestCount", e.target.value)}
                className="w-full border border-gray-200 rounded-xl px-4 py-3 text-[13px] outline-none focus:border-yellow-400" />
            </div>
          </div>
          <div>
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          <div className="flex justify-between items-start">
            <div>
              <div className="text-[16px] font-bold text-gray-900">{booking.guestName}</div>
              <div className="text-[12px] text-gray-400">{booking.guestPhone} · {booking.guestEmail}</div>
            </div>
          ...
          {booking.checkInCode && (
            <div className="bg-yellow-50 rounded-xl p-3 flex items-center justify-between">
              <span className="text-[12px] font-bold text-gray-600">Check-in Code</span>
              <span className="font-mono text-[15px] font-black" style={{ color: "var(--navy)" }}>{booking.checkInCode}</span>
            </div>
          )}
          <div className="bg-gray-50 rounded-xl p-4 space-y-2.5">
            {[
              { label: "Unit", value: `${booking.unit.unitNumber} — ${booking.unit.property.name}` },
              ...
              { label: "Source", value: booking.source.replace("_", ".") },
              { label: "Nightly Rate", value: formatCurrency(booking.nightlyRate) },
            ].map(({ label, value }) => (
          ...
          {booking.specialRequests && (
            <div className="text-[12.5px] text-gray-600 bg-blue-50 rounded-xl p-3">
...
function EditBookingModal({ booking, onClose }: { booking: Booking; onClose: () => void }) {
  const router = useRouter();
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ status: booking.status, specialRequests: booking.specialRequests ?? "" });
  const set = (k: string, v: string) => setForm((f) => ({ ...f, [k]: v }));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between" style={{ background: "var(--navy)" }}>
          <div className="text-[15px] font-bold text-white">Update Booking</div>
          <button onClick={onClose} className="text-white/60 hover:text-white"><X size={20} /></button>
        </div>
        <div className="p-6 space-y-4">
          <div>
...
export function ShortletsTopbar() {
  const [open, setOpen] = useState(false);
  return (
    <>
      <Topbar title="Shortlet Management" action={{ label: "New Booking", onClick: () => setOpen(true) }} />
...
export function BookingRowActions({ booking }: { booking: Booking }) {
  ...
  return (
    <>
      ...
      {detail && <BookingDetailModal booking={booking} onClose={() => setDetail(false)} />}
