...
type LiveKPIs = {
  ...
  propertyPerf: { name: string; revenue: number; units: number; leased: number }[];
};

export function AnalyticsClient({ live }: { live: LiveKPIs }) {
  const occRate = live.totalUnits > 0 ? Math.round((live.activeleases / live.totalUnits) * 100) : 0;

  const kpis = [
    { label: "Total Properties", value: String(live.totalProperties), change: 0, icon: <Building2 size={16} />, color: "var(--navy)" },
    { label: "Total Revenue (collected)", value: formatCurrency(live.totalRevenue), change: +8.4, icon: <CreditCard size={16} />, color: "var(--gold)" },
    { label: "Occupancy Rate", value: `${occRate}%`, change: +2.1, icon: <TrendingUp size={16} />, color: "var(--emerald)" },
    { label: "Active Tenants", value: String(live.totalTenants), change: 0, icon: <Users size={16} />, color: "#3B82F6" },
    { label: "Total Units", value: String(live.totalUnits), change: 0, icon: <Building2 size={16} />, color: "#8B5CF6" },
    { label: "Open Work Orders", value: String(live.openWorkOrders), change: 0, icon: <Wrench size={16} />, color: "#EF4444" },
    { label: "Collection Rate", value: `${live.collectionRate}%`, change: +2.1, icon: <TrendingUp size={16} />, color: "var(--emerald)" },
    { label: "Overdue Amount", value: formatCurrency(live.overdueAmount), change: 0, icon: <TrendingDown size={16} />, color: live.overdueAmount > 0 ? "#EF4444" : "var(--emerald)" },
  ];

  const propertyPerf = live.propertyPerf.map((p) => ({
    name: p.name,
    revenue: p.revenue,
    occ: p.units > 0 ? Math.round((p.leased / p.units) * 100) : 0,
  }));
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100">
                    {["Property", "Monthly Revenue", "Occupancy", "Performance Bar", "vs 90% Target"].map((h) => (
                      ...
                      <tr key={p.name} className="border-b border-gray-50 hover:bg-gray-50/50">
                        ...
                        <td className="px-4 py-3">
                          <div className="w-32 h-2 bg-gray-100 rounded-full overflow-hidden">
                            <div className="h-full rounded-full" style={{ width: `${p.occ}%`, background: p.occ >= 80 ? "var(--emerald)" : p.occ >= 60 ? "var(--gold)" : "#EF4444" }} />
                          </div>
                        </td>
                        <td className="px-4 py-3">
