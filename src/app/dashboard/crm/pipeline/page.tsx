...
type Deal = { id: string; title: string; contactName: string; value: number; stage: string; probability: number; dueDate: string | null; notes: string | null };

function AddDealModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ title: "", contactName: "", value: "", stage: "Enquiry", probability: "30", dueDate: "", notes: "" });
  const set = (k: string, v: string) => setForm((f) => ({ ...f, [k]: v }));

  async function save() {
    if (!form.title || !form.contactName) return;
    setSaving(true);
    await fetch("/api/crm/deals", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
    setSaving(false);
    onCreated();
    onClose();
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md mx-4 overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          {[{ label: "Deal Title *", key: "title" }, { label: "Contact Name *", key: "contactName" }].map(({ label, key }) => (
            <div key={key}>
...
export default function PipelinePage() {
  ...
  async function moveDeal(id: string, stage: string) {
    ...
    await fetch(`/api/crm/deals/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ stage }) });
  }
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            { label: "Active Deals", value: deals.filter((d) => !["Lost", "Completed"].includes(d.stage)).length, color: "var(--navy)" },
        ...
        {loading ? (
          <div className="text-center text-gray-400 py-12 text-[13px]">Loading pipeline…</div>
        ) : (
              ...
              return (
                <div key={stage} className="flex-shrink-0 w-52">
                  ...
                  <div className="space-y-2 min-h-[60px]">
                    {stageDeals.map((deal) => (
                      <div key={deal.id} className="bg-white rounded-xl border border-gray-100 p-3 shadow-sm group relative">
                        <button onClick={() => deleteDeal(deal.id)}
                          className="absolute top-2 right-2 w-5 h-5 rounded flex items-center justify-center opacity-0 group-hover:opacity-100 hover:bg-red-100 transition-opacity">
                          <Trash2 size={10} className="text-red-400" />
                        </button>
                        <div className="text-[12px] font-bold text-gray-900 mb-1 leading-tight pr-4">{deal.title}</div>
                        <div className="text-[10.5px] text-gray-400 mb-2">{deal.contactName}</div>
      ...
      {adding && <AddDealModal onClose={() => setAdding(false)} onCreated={load} />}
    </div>
  );
}
